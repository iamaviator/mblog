    ...
    async function fetchPage() {
      try {
        ...
        if (res.ok) {
          ...
          setPage(pageData);
          const html = await markdownToHtml(pageData.content);
          setHtmlContent(html);
        }
      } catch (error) {
    ...
    <>
      ...
      <aside style={{ borderColor: 'var(--border-color)', backgroundColor: 'var(--bg-secondary)' }} className="hidden md:flex md:w-64 md:h-screen md:sticky md:top-0 border-r flex-col justify-between px-6 py-8">
        <div>
          ...
          <nav className="space-y-3 text-sm">
            ...
            <Link href="/about" className="block font-semibold">About</Link>
          </nav>
        </div>
        <ThemeSwitcher />
      </aside>
      ...
      <div className="flex-1 min-w-0">
        ...
        <main className="max-w-4xl mx-auto px-4 md:px-10 py-10">
          <Link href="/" style={{ color: 'var(--accent)' }} className="text-sm mb-6 inline-block">
            ← Back to home
          </Link>
